...
dont_write = "{}, вы не писали заявку на РП-сессию." \
             "\nЗаполнение заявки: {}заявка `<ник>` `<роль>` <`станция`(для маневровых и дсцп писать в одно слово)/`номер маршрута`(для машиниста)/`линия`(для дцх)> <`линия`(дцх здесь ничего указывать не надо)>"
